...
오픈스택에는 현재 서버의 상황을 보여주는 server list명령어가 있다.<br>
해당 명령어를 매개변수에 넣고 실행하면 다음과 같은 결과가 출력된다.<br>
```
+--------------------------------------+------+--------+--------------------------------------------------------------------------+--------------------------+---------+
| ID                                   | Name | Status | Networks                                                                 | Image                    | Flavor  |
+--------------------------------------+------+--------+--------------------------------------------------------------------------+--------------------------+---------+
| 579eeac2-63ce-4c50-8021-511f0b4fc2b8 | test | ACTIVE | private=10.0.0.10, 192.168.100.217, fd47:3f02:76a0:0:f816:3eff:fe94:1a78 | N/A (booted from volume) | m1.nano |
+--------------------------------------+------+--------+--------------------------------------------------------------------------+--------------------------+---------+

```
위 결과에 Created라는 옵션을 추가하여 출력하려면 어떻게 해야할까?<br>
...
```
server.py를 보면 Listserver클래스가 보이는데 2218행의 take_action을 보면 파싱을 통해 네트워크에서 받아온 정보들을 분류하여 2369행-2380행의 columns와 column_headers로 출력할 내용과 이름을 저장하는것을 볼 수 있다.
```
columns = (
    'id',
    'name',
...
```
따라서 다음과 같이 columns와 columns_headers에 created, Created를 추가해주면
```
columns = (
    'id',
    'name',
...
```
정상적으로 Created가 출력되는 모습을 볼 수 있다.
```
+--------------------------------------+------+--------+----------------------+--------------------------------------------------------------------------+--------------------------+---------+
| ID                                   | Name | Status | Created              | Networks                                                                 | Image                    | Flavor  |
+--------------------------------------+------+--------+----------------------+--------------------------------------------------------------------------+--------------------------+---------+
| 579eeac2-63ce-4c50-8021-511f0b4fc2b8 | test | ACTIVE | 2022-05-21T07:59:00Z | private=10.0.0.10, 192.168.100.217, fd47:3f02:76a0:0:f816:3eff:fe94:1a78 | N/A (booted from volume) | m1.nano |
+--------------------------------------+------+--------+----------------------+--------------------------------------------------------------------------+--------------------------+---------+
```
...
+---------------------------------------------+-------------------------------+
  ```
해당 버그는 현재 gerrit에 다음과 같은 제목으로 올라와있는 모습을 볼 수 있다.<br>
[Fix bug that configuration show --mask cannot masking the password & token](https://review.opendev.org/c/openstack/python-openstackclient/+/811005)
